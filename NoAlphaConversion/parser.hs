...
happyExpList = Happy_Data_Array.listArray (0,52) ([6848,3424,0,840,0,16,0,4,24602,13,16384,16391,2048,0,0,6656,3328,0,64,420,0,0
	])

{-# NOINLINE happyExpListPerState #-}
happyExpListPerState st =
    token_strs_expected
  where token_strs = ["error","%dummy","%start_parse","start","line","exp","NL","LET","'='","VAR","'.'","'\955'","'('","')'","%eof"]
...
happyReduction_5 (_ `HappyStk`
	...
	_ `HappyStk`
	(HappyTerminal (Term happy_var_2)) `HappyStk`
	_ `HappyStk`
	happyRest)
	 = HappyAbsSyn5
...
happyReduction_6 (HappyTerminal (Term happy_var_1))
	 =  HappyAbsSyn6
		 (Var happy_var_1
	)
...
happyNewToken action sts stk (tk:tks) =
	...
	Let -> cont 8;
	Eq -> cont 9;
	Term happy_dollar_dollar -> cont 10;
	Dot -> cont 11;
	Lambda -> cont 12;
	OpenPar -> cont 13;
	ClosedPar -> cont 14;
